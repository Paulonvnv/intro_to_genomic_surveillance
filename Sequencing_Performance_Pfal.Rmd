...
cigar_object = read_cigar_tables(cigar_files = cigar_files, sample_id_pattern = "ID")
```
...
Pfal_metadata = read.csv('~/Documents/Github/intro_to_genomic_surveillance/docs/data/Pfal_example/Pfal_metadata.csv')

cigar_object@metadata = left_join(
  cigar_object@metadata,
  Pfal_metadata, by = 'Sample_id'
)
...
```{r}
markers = read.csv('~/Documents/Github/intro_to_genomic_surveillance/docs/reference/Pfal_3D7/markers.csv')
```
...
ampseq_object = cigar2ampseq(cigar_object, markers = markers, min_abd = 10, min_ratio = 0.1, remove_controls = T)
```
...
Let's do it by Municipality

```{r}
ReadDepth_coverage = get_ReadDepth_coverage(ampseq_object_abd1, variable = 'Subnational_level2')
...
plot_precentage_of_samples_over_min_abd_byCountry = sample_performance %>%
      summarise(AmpRate5 = round(100*sum(AmpRate >= .05)/n(), 1),
                ...
                AmpRate95 = round(100*sum(AmpRate >= .95)/n(), 1),
                AmpRate100 = round(100*sum(AmpRate >= 1)/n(), 1),
                .by = c(Threshold, Subnational_level2)
      ) %>%
      pivot_longer(cols = paste0('AmpRate', seq(5, 100, 5)),
                   values_to = 'Percentage',
                   names_to = 'AmpRate') %>%
...
```{r, include = FALSE}
fig3.height = 3*ceiling(length(unique(plot_precentage_of_samples_over_min_abd_byCountry$data$Subnational_level2))/3)
```
...
ref_fasta = '~/Documents/Github/intro_to_genomic_surveillance/docs/reference/Pfal_3D7/pf3d7_ref_updated_v3.fasta'
...
```{r}
ReadDepth_coverage = get_ReadDepth_coverage(ampseq_object, variable = 'Subnational_level2')
```
...
all_loci_amplification_rate = locus_amplification_rate(ampseq_object, threshold = 0.65, strata = 'Subnational_level2', update_loci = F)
...
ampseq_object = locus_amplification_rate(ampseq_object, threshold = 0.65, strata = 'Subnational_level2', update_loci = T, based_on_strata = F)
...
```{r}
samples_amplification_rate = sample_amplification_rate(ampseq_object, threshold = 0.8, strata = 'Subnational_level2', update_samples = F)
```
...
```{r}
ampseq_object = sample_amplification_rate(ampseq_object, threshold = 0.5, strata = 'Subnational_level2', update_samples = T)
```
...
```{r}
write_ampseq(ampseq_object, name = '~/Documents/Github/intro_to_genomic_surveillance/docs/data/Pfal_example/Pfal_ampseq_filtered.xlsx', format = 'excel')
```

```{r}
test = read_ampseq(file = '~/Documents/Github/intro_to_genomic_surveillance/docs/data/Pfal_example/Pfal_ampseq_filtered.xlsx', 
                   format = 'excel')
```
