...
read_cigar_tables = function(paths = NULL,
  ...
  if(!is.null(paths)&is.null(cigar_files)){
    ...
    for(file in 1:length(cigar_files)){
      ...
      if(!is.null(asv_table_files) & !is.null(asv2cigar_files) & !is.null(asv_seq_files) & !is.null(zero_read_sample_list)){
...
setGeneric("frac_ofHet_pAlt_byAllele", function(obj = NULL, ref_fasta = NULL
                                                ) standardGeneric("frac_ofHet_pAlt_byAllele"))

setMethod("frac_ofHet_pAlt_byAllele", signature(obj = "ampseq"),
          
          function(obj = NULL, ref_fasta = NULL
                   ){
            ...
            for(mhap in mhaps$amplicon){
              ...
              homopolymers = unlist(str_extract_all(as.character(ref_sequences[[mhap]]), '(A{5}|T{5}|G{5,}|C{5,})'))
              homopolymers_location = str_locate_all(as.character(ref_sequences[[mhap]]), '(A{5}|T{5}|G{5,}|C{5,})')
            ...
            for(mhap in 1:ncol(gt)){
              ...
              if(length(alleles) != 0){
                ...
                allele_count_frac_ofHet_pAlt_temp[['nSNVs_ij']] = as.integer(allele_count_frac_ofHet_pAlt_temp[['nSNVs_ij']])
                ...
                allele_count_frac_ofHet_pAlt_temp[['dSNVs_ij']] = as.numeric(allele_count_frac_ofHet_pAlt_temp[['dSNVs_ij']])
                
                allele_count_frac_ofHet_pAlt_temp[['SNV_in_homopolymer']] = as.logical(allele_count_frac_ofHet_pAlt_temp[['SNV_in_homopolymer']])
                allele_count_frac_ofHet_pAlt_temp[['INDEL_in_homopolymer']] = as.logical(allele_count_frac_ofHet_pAlt_temp[['INDEL_in_homopolymer']])
                
                allele_count_frac_ofHet_pAlt_temp[['flanking_INDEL']] = as.logical(as.integer(allele_count_frac_ofHet_pAlt_temp[['flanking_INDEL']]))
                
                allele_count_frac_ofHet_pAlt_temp[['p_ij']] = allele_count_frac_ofHet_pAlt_temp[['P_ij']]/nrow(gt)
            ...
            allele_count_frac_ofHet_pAlt %<>% mutate(SNV_in_homopolymer_pattern = case_when(
            ...
            allele_count_frac_ofHet_pAlt %<>% dplyr::select(MHap,
                                                          Allele,
                                                          P_ij,
                                                          H_ij,
                                                          H_ijminor,
...
setGeneric("mask_alt_alleles", function(obj = NULL, ref_fasta = NULL, mask_formula = "dVSITES_ij > 0.3") standardGeneric("mask_alt_alleles"))

setMethod("mask_alt_alleles", signature(obj = "ampseq"),
          
          function(obj = NULL,  ref_fasta = NULL, mask_formula = "dVSITES_ij > 0.3"){
            ...
            if(grepl("(h_ij|h_ijminor|p_ij|P_ij|H_ij|H_ijminor|nVSITES_ij|dVSITES_ij|nSNVs_ij|dSNVs_ij|nINDELs_ij|dINDELs_ij|flanking_INDEL|SNV_in_homopolymer|INDEL_in_homopolymer)(<|>|!|=)+", mask_formula)){
              stop("All mathematical and logical operators must be separated by blank spaces in mask_formula")
            }
            ...
            if(grepl("SNV_in_homopolymer ", mask_formula)){
              ...
              if(!is.na(mask_filter)){
                ...
                mask_formula = gsub("SNV_in_homopolymer ", "allele_count_frac_ofHet_pAlt_temp[['SNV_in_homopolymer']] ", mask_formula)
              }else{
            ...
            if(grepl("INDEL_in_homopolymer ", mask_formula)){
              ...
              if(!is.na(mask_filter)){
                ...
                mask_formula = gsub("INDEL_in_homopolymer ", "allele_count_frac_ofHet_pAlt_temp[['INDEL_in_homopolymer']] ", mask_formula)
              }else{
            ...
            if(grepl("dSNVs_ij ", mask_formula)){
              ...
              if(!is.na(mask_filter)){
                ...
                mask_formula = gsub("dSNVs_ij ", "allele_count_frac_ofHet_pAlt_temp[['dSNVs_ij']] ", mask_formula)
              }else{
            ...
            if(grepl("nSNVs_ij ", mask_formula)){
              ...
              if(!is.na(mask_filter)){
                ...
                mask_formula = gsub("nSNVs_ij ", "allele_count_frac_ofHet_pAlt_temp[['nSNVs_ij']] ", mask_formula)
              }else{
            ...
            if(length(mask_formula_check) > 0){
              ...
              stop("Execution halted, revise mask_filter argument.\nPossible filters are:\nh_ij, h_ijminor, p_ij, P_ij, H_ij, H_ijminor, nVSITES_ij, dVSITES_ij, nSNVs_ij, dSNVs_ij, nINDELs_ij, dINDELs_ij, flanking_INDEL, SNV_in_homopolymer, INDEL_in_homopolymer")
            }
            ...
            for(mhap in 1:ncol(gt)){
              ...
              if(length(alleles) != 0){
                ...
                allele_count_frac_ofHet_pAlt_temp[['flanking_INDEL_pattern']] = unlist(sapply(allele_count_frac_ofHet_pAlt_temp[['flanking_INDEL_pattern']],
                                                                                       function(pattern){
                ...
                allele_count_frac_ofHet_pAlt_temp[['INDEL_in_homopolymer_replacement']] = unlist(sapply(allele_count_frac_ofHet_pAlt_temp[['INDEL_in_homopolymer_replacement']],
                ...
                if(grepl('SNV_in_homopolymer',mask_formula)){
                  ...
                  if(length(replaced_alleles) > 0){
                    ...
                    mask_formula2 = str_extract(mask_formula,
                                                "allele_count_frac_ofHet_pAlt_temp\\[\\['SNV_in_homopolymer'\\]\\] (=|!)+ (TRUE|FALSE)")
                  ...
                  if(length(replaced_alleles) > 0){
                    ...
                    mask_formula2 = str_extract(mask_formula,
                                                "allele_count_frac_ofHet_pAlt_temp\\[\\['INDEL_in_homopolymer'\\]\\] (=|!)+ (TRUE|FALSE)")
