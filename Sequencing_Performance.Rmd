...
n_off_target_alleles = off_target_stats[off_target_stats$dVSITES_ij >= 0.25,][['Allele']]
...
```{r}
ampseq_object@gt = mask_alt_alleles(ampseq_object, mask_formula = 'dVSITES_ij >= 0.25', ref_fasta = ref_fasta)
```
...
while(length(n_flanking_INDEL_alleles) > 0){
  ampseq_object@gt = mask_alt_alleles(ampseq_object, mask_formula = 'flanking_INDEL == TRUE',
                                      ref_fasta = ref_fasta)
  
  flanking_INDEL_stats2 = frac_ofHet_pAlt_byAllele(ampseq_object, 
                                            ref_fasta = ref_fasta)
...
```{r}
homopolymers_stats = frac_ofHet_pAlt_byAllele(ampseq_object, 
                                            ref_fasta = ref_fasta)
```
...
plot_homopolymers_stats = homopolymers_stats %>%
      ...
      theme_bw()+
      scale_color_continuous(type = 'viridis')+
      facet_grid(vsites_in_homopopylmers ~ h_ijminor_cat)+
      labs(x = 'Alternative allele prev. (p_ij)',
           y = 'h_ij (H_ij/P_ij)',
           color = 'h_ijminor')
...
n_snv_in_homopolymer_alleles = homopolymers_stats[
  homopolymers_stats$SNV_in_homopolymer == TRUE, ][['Allele']]
...
```{r}
n_indel_in_homopolymer_alleles = homopolymers_stats[homopolymers_stats$INDEL_in_homopolymer == TRUE, ][['Allele']]
...
```{r}
ampseq_object@gt = mask_alt_alleles(ampseq_object, mask_formula = 'SNV_in_homopolymer == TRUE | INDEL_in_homopolymer == TRUE', ref_fasta = ref_fasta)
```
