...
### Off-target products----
off_target_stats = frac_ofHet_pAlt_byAllele(ampseq_object, ref_fasta = amplicon_fasta)
...
if(length(n_off_target_alleles) > 0){
  ...
  ampseq_object@gt = mask_alt_alleles(ampseq_object, mask_formula = off_target_formula, ref_fasta = amplicon_fasta)
...
flanking_INDEL_stats = frac_ofHet_pAlt_byAllele(ampseq_object, ref_fasta = amplicon_fasta)
...
if(length(n_flanking_INDEL_alleles) > 0){
  ...
  # mask flanking_INDEL
  ampseq_object@gt = mask_alt_alleles(ampseq_object, mask_formula = flanking_INDEL_formula, ref_fasta = amplicon_fasta)
}else{
...
PCR_errors_stats = frac_ofHet_pAlt_byAllele(ampseq_object, ref_fasta = amplicon_fasta)
...
if(length(n_PCR_errors_alleles) > 0){
  ...
  gt_masked = mask_alt_alleles(ampseq_object, mask_formula = PCR_errors_formula, ref_fasta = amplicon_fasta)
